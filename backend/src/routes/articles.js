...
fastify.get('/aticles/:id', async (request, reply) => {
    const id = request.params.id;
    const collection = fastify.mongo.db.collection('aticles');
    const result = await collection.findOne({ _id: ObjectId(id) });

    return result;
})

fastify.delete('/aticles/:id', async (request, reply) => {
    const id = request.params.id;
...
fastify.patch('/aticles/:id',{ schema: { body: patchCourseBody } },  async (request, reply) => {
    const id = request.params.id;
    //const {id} = request.params;
    const collection = fastify.mongo.db.collection('courses');
